...
NEWS_SOURCE_NAME = "ESPN"
NEWS_SOURCE_URL = "https://www.espn.com/nfl/"
...
def get_espn_nfl_news(cache={}):
    """Whole-league NFL headlines from ESPN's public RSS feed, cached 30min
    so we're not hammering ESPN every time a player page loads. Per-player
    news is derived by filtering this list by name -- ESPN doesn't publish
    a per-player feed."""
    now = time.time()
    if "items" in cache and now - cache.get("time", 0) < 1800:
        return cache["items"]
    try:
        r = requests.get(ESPN_NFL_RSS, timeout=10, headers={"User-Agent": "Mozilla/5.0"})
        r.raise_for_status()
        ...
        # ESPN hiccup / feed format change -- fail quietly and keep showing
        # whatever we last had (empty on a cold start), never break the page.
...
def get_team_depth_chart(team, all_players):
    ...
    for slot in groups:
        groups[slot].sort(key=lambda x: x["order"])
    return [
        {"slot": slot, "base": re.sub(r"\d+$", "", slot), "players": groups[slot]}
        for slot in sorted(groups.keys(), key=_depth_slot_sort_key)
    ]
...
def _bootstrap_stats_if_empty():
    """Runs once at app startup. If player_stats is empty (a fresh
    database, or one that never got backfilled), automatically kicks off
    a full backfill in a background thread -- no manual URL visit or
    workflow click required. Runs in the background specifically so it
    can never block a page load or cause a request-timeout issue."""
    if not DATABASE_URL:
        return
    try:
        conn = get_db()
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) AS c FROM player_stats")
                count = cur.fetchone()["c"]
        finally:
            conn.close()
    except Exception:
    ...
    def _run():
        for yr in range(2015, current_nfl_season() + 1):
            try:
                sync_season_to_db(yr)
            except Exception:
                pass
...
def player_detail():
    ...
    return render_template_string(
        ...
        news_source_name=NEWS_SOURCE_NAME, news_source_url=NEWS_SOURCE_URL,
    )
...
def api_sync_stats():
    """Protected endpoint the scheduled sync job (and the one-time
    backfill workflow) calls. Not meant for browsers -- requires the
    site password as a shared secret so a stranger can't trigger a bunch
    of expensive Sleeper fetches. Deliberately does ONE season per call
    and nothing more -- looping through many seasons inside a single web
    request is exactly what caused the site to time out and appear
    completely broken before; that looping belongs in the calling
    workflow (which has no such time limit), not in here."""
    if request.args.get("secret") != SITE_PASSWORD:
        return jsonify({"ok": False, "error": "unauthorized"}), 401
    season = request.args.get("season", default=int(SEASON), type=int)
    rows_saved = sync_season_to_db(season)
    return jsonify({"ok": True, "season": season, "rows_saved": rows_saved})
  ...
  <div class="panel">
    ...
    {% if news %}
      ...
      <div class="news-item">
        ...
        <div class="news-meta">{{ n.ago }}{% if n.ago %} &middot; {% endif %}via {{ news_source_name }}</div>
        {% if n.desc %}<div class="news-desc">{{ n.desc }}</div>{% endif %}
      </div>
      {% endfor %}
    {% else %}
      <p class="muted" style="margin-top:10px;">No recent {{ news_source_name }} headlines mention {{ p.name }} right now.</p>
    {% endif %}
    <div class="news-credit">News via <a href="{{ news_source_url }}" target="_blank" rel="noopener">{{ news_source_name }}</a> &middot; headline &amp; summary only, links back to the original article.</div>
  </div>
