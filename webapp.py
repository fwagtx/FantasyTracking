...
def get_season_stats(season, cache={}):
    ...
    from our own database (fast, no Sleeper calls) -- populated by the
    scheduled sync job. Short in-memory cache on top since this can be
    called many times per request. Falls back to a one-time live Sleeper
    fetch (which saves itself to the DB) only if this season has never
    been synced -- self-healing without needing to wait on the schedule."""
    now = time.time()
    ...
    if not agg:
        # Never synced yet -- fetch live once and save it, so this branch
        # is never hit again for this season.
        weekly = fetch_season_stats_from_sleeper(season)
        for pid, weeks in weekly.items():
            ...
            for week, w in weeks.items():
                p_entry["games"] += 1
                p_entry["fpts"] += w["pts"]
                p_entry["weeks"][week] = w["pts"]
                p_entry["off_snp_total"] += w["off_snp"]
                p_entry["tm_off_snp_total"] += w["tm_off_snp"]
...
def api_sync_stats():
    ...
    browsers -- requires the site password as a shared secret so a
    stranger can't trigger a bunch of expensive Sleeper fetches."""
...
<style>
  ...
  @media (max-width: 760px) {
    ...
    table.rk-table{ display:block; overflow-x:auto; white-space:nowrap; }
