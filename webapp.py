...
team in the league using real dynasty trade values from FantasyCalc's
public API. Includes a Rankings page, a photo-based live-search Trade
Calculator (Dynasty/Redraft + 1QB/Superflex), real ADP on player pages via
Fantasy Football Calculator's public API, and clickable player profiles.
Player photos come from Sleeper's own official headshot CDN.
...
def search_players(query, limit=8):
    ...
    for sid, p in all_players.items():
        ...
        if q in full.lower():
            matches.append({"sid": sid, "name": full, "position": p.get("position"), "team": p.get("team") or "FA"})
    matches.sort(key=lambda r: (not r["name"].lower().startswith(q), r["name"]))
    return matches[:limit]

# ---------------- FantasyCalc (real dynasty/redraft trade values) ----------------

def get_fantasycalc_values(num_qbs, is_dynasty=True, cache={}):
    """sleeper_id -> value/rank data, cached 1hr per (format, dynasty-vs-redraft)."""
    ...
    cache[key] = {"data": mapped, "time": now}
    return mapped


def get_adp_data(cache={}):
    """player full name (lowercase) -> overall ADP, from Fantasy Football
    Calculator's public API. Best-effort -- returns {} on any failure."""
    now = time.time()
...
def player_detail():
    ...
    info = {
        ...
        "age": p.get("age"), "years_exp": p.get("years_exp"), "college": p.get("college"),
        ...
        "adp": adp_data.get(full_name.lower()),
    }
    return render_template_string(PLAYER_HTML, p=info, username=username)
...
def api_player_search():
    ...
    for r in results:
        r["photo"] = player_photo_url(r["sid"])
...
def trade_calculator():
    ...
    if side1_ids or side2_ids:
        all_players = get_all_players()
        fc_values = get_fantasycalc_values(num_qbs, is_dynasty)
        ...
        def build_side(ids):
            ...
            for sid in ids:
                ...
                if not p:
                    continue
                val = fc_values.get(sid, {}).get("value", 0)
                items.append({
...
<main><div class="wrap" style="max-width:640px;">
  ...
  <div class="panel">
    ...
    <div class="legend-box" style="margin-top:20px;">
      ...
      <div style="margin-top:8px;">Dynasty/redraft values: FantasyCalc consensus market. ADP: Fantasy Football Calculator's public redraft ADP data. We don't currently pull weekly game logs -- ask if you want that added next.</div>
    </div>
  </div>
</div></main>
"""
...
<main><div class="wrap">
  <div class="panel">
    <p class="eyebrow">Real dynasty &amp; redraft values &middot; FantasyCalc</p>
    ...
    <div class="trade-cols">
      <div>
        ...
        <div class="search-wrap">
          <input type="text" id="search1" placeholder="Type a player name&hellip;" autocomplete="off">
      ...
      <div>
        ...
        <div class="search-wrap">
          <input type="text" id="search2" placeholder="Type a player name&hellip;" autocomplete="off">
    ...
    <div class="trade-result">
      ...
      <p class="muted">Draft picks aren't priced yet -- only rostered NFL players. Ask if you want picks added.</p>
    </div>
    {% endif %}
  </div>
</div></main>
...
function wireSearch(side) {
  ...
  input.addEventListener('input', () => {
    ...
    debounceTimer = setTimeout(async () => {
      const resp = await fetch('/api/player-search?q=' + encodeURIComponent(q));
      const data = await resp.json();
      dropdown.innerHTML = '';
      data.results.forEach(r => {
        const item = document.createElement('div');
        item.className = 'search-dropdown-item';
        item.innerHTML = `<img src="${r.photo}" onerror="this.style.visibility='hidden'"><span>${r.name} <span class="muted">${r.position} &middot; ${r.team}</span></span>`;
        item.onclick = () => addPlayer(side, {sid: r.sid, name: r.name, position: r.position, team: r.team, photo: r.photo, value: null});
