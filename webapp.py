...
def player_detail():
    ...
    # We keep the last 4 seasons available -- not literally "since the
    # player entered the league" for veterans, since that could mean 10+
    # years x 18 weeks of requests per profile view. Honest tradeoff.
    available_seasons = list(range(current_season - 3, current_season + 1))
    ...
    info = {
        ...
        "adp": adp_data.get(full_name.lower()), "ppg": current_ppg,
    }
...
<main><div class="wrap" style="max-width:700px;">
  ...
  <div class="panel">
    ...
    <div class="legend-box" style="margin-top:18px;">
      ...
      <div style="margin-top:8px;">Dynasty/redraft values: FantasyCalc consensus market. ADP: Fantasy Football Calculator's public redraft ADP (one consensus number, not broken out per platform). Weekly/season stats: Sleeper.</div>
    </div>
    {% endif %}
    ...
    <p class="muted" style="margin-top:12px;">Showing the last {{ career_rows|length }} seasons -- not necessarily the player's entire career, to keep the page fast.</p>
    {% endif %}
  </div>
</div></main>
"""
