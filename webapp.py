...
def get_adp_data(cache={}):
    ...
    try:
        ...
        by_name = {p.get("name", "").strip().lower(): p.get("adp") for p in players if p.get("name")}
        cache["data"] = by_name
        cache["time"] = now
        return by_name
    except Exception:
...
def api_submit_vote():
    ...
    for sid, label in data.get("votes", {}).items():
        entry = vote_counts.setdefault(sid, {"keep": 0, "trade": 0, "cut": 0})
        if label in entry:
            entry[label] += 1
...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  .vote-btn.keep.selected{ background:var(--good-wash); color:var(--good); }
  .vote-btn.trade.selected{ background:var(--paper-sunken); color:var(--ink); }
...
<div class="vote-overlay" id="voteOverlay">
  <div class="vote-modal">
    ...
    <p class="sub"><b>Keep</b> the most valuable, <b>Trade</b> the middle, <b>Cut</b> the least valuable.</p>
  ...
  try {
    ...
    voteTrio.forEach(p => {
      ...
      card.innerHTML = `
        ...
        <div class="vote-btns">
          <button class="vote-btn keep" onclick="setVoteLabel('${p.sid}','keep',this)">KEEP</button>
          <button class="vote-btn trade" onclick="setVoteLabel('${p.sid}','trade',this)">TRADE</button>
