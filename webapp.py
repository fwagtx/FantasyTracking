...
def get_season_stats(season, cache={}):
    ...
    endpoint. Cached 1hr. Weeks are fetched in parallel since this is a
    network-bound wait, not a CPU-bound one -- makes a big difference on
    a cold cache. Best-effort -- skips any week that errors."""
    now = time.time()
...
<div class="rk-modal-overlay" id="filterModal">
  <div class="rk-modal">
    ...
    <div class="rk-slider-row">
      <label><span>Min Age</span><span id="ageVal">0</span></label>
      <input type="range" id="ageSlider" min="0" max="40" value="0">
    </div>
    <div class="rk-slider-row">
...
const RK_DATA = [
  ...
  {sid:{{ r.sid|tojson }}, photo:{{ r.photo|tojson }}, name:{{ r.name|tojson }}, position:{{ r.position|tojson }},
   ...
   fpts_per_game:{{ r.fpts_per_game|tojson }}, position_rank:{{ r.position_rank|tojson }}, value:{{ r.value|tojson }},
...
let state = {
  ...
  sortDir: 1,
  minAge: 0, minGames: 0, minValue: 0,
};
...
function getFiltered() {
  let rows = RK_DATA.filter(r => {
    ...
    if (r.age !== null && r.age < state.minAge) return false;
...
const OVERALL_COLS = [
  ...
  {key:'team', label:'TM'}, {key:'age', label:'Age'}, {key:'games', label:'GP'},
  {key:'fpts_per_game', label:'FPTS/G'}, {key:'position_rank', label:'Pos Rank'}, {key:'overall_rank', label:'Ovr Rank'},
];
const POSITION_COLS = [
  {key:'overall_rank', label:'#'}, {key:'name', label:'Player'}, {key:'age', label:'Age'},
  ...
  rows.forEach(r => {
    ...
    cols.forEach(c => {
      if (c.key === 'name') {
        ...
        cells += `<td>${statCell(r.age, percentileClass(ageVals, r.age, false))}</td>`;
      } else if (c.key === 'games') {
...
['age','games','value'].forEach(key => {
  const slider = document.getElementById(key + 'Slider');
  const label = document.getElementById(key + 'Val');
  slider.addEventListener('input', () => { label.textContent = slider.value; });
});
document.getElementById('applyFilters').addEventListener('click', () => {
  state.minAge = parseInt(document.getElementById('ageSlider').value);
...
document.getElementById('resetFilters').addEventListener('click', () => {
  ...
  state.minAge = 0; state.minGames = 0; state.minValue = 0;
  render();
});
